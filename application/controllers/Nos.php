...
class Nos extends CI_Controller
{
    ...
    function get()
    {
        ...
        foreach ($list as $field) {
            ...
            $nos_audit = $this->db->where('id_nos', $field->id_nos)->get('nos_audit')->result_array();
            $nos = $this->Nos_model->getNosComplete($field->id_nos);
            $id_panel_sub = explode(',', $nos['id_panel_sub']);
            $nos_data = $this->db->where_in('id_panel_sub', $id_panel_sub)->get('nos_data')->result_array();

            $dealer = "<strong>$field->nama_dealer</strong><br><span>Tipe Panel : <strong>$field->nama_panel</strong></span>";

            if (count($nos_audit) == 0) {
                $status = '<span class="badge rounded-pill bg-danger font-size-12">Audit Belum Dilakukan</span>';
            ...
            } else if (count($nos_audit) < count($nos_data)) {
    ...
    public function sub_item($id_nos, $id_panel_sub, $item)
    {
        if ($this->session->userdata('email')) {
            ...
            $data['header'] = 'temp/header';
            $data['nos_data'] = $this->Nos_model->sub_item($url, decrypt_url($id_panel_sub));
            $data['nos'] = $this->Nos_model->getNosComplete(decrypt_url($id_nos));
            if ($this->session->userdata('id_regional') == 0 || $this->session->userdata('id_regional') == $data['nos']['id_regional']) {

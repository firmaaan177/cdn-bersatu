                    ...
                    <div class="row">
                        <div class="form-group mb-3 col-md-12">
                            <label class="form-control-label">
                                Nama Panel <span class="text-danger">*</span>
                            </label>
                            <input type="text" class="form-control" name="nama_panel" id="nama_panel">
                        </div>
                    </div>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" id="update" enctype="multipart/form-data">
                    <input type="hidden" name="id_panel" id="id_panel">
                    <div class="row">
                        <div class="form-group mb-3 col-md-12">
                            <label class="form-control-label">
                                Nama Panel <span class="text-danger">*</span>
                            </label>
                            <input type="text" class="form-control" name="nama_panel" id="nama_panel2">
                        </div>
                    </div>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    function tampil() {
        ...
        $('#datatables').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('panel/get') ?>",
                "type": "POST"
            },
...
<script>
    $(document).ready(function() {
        ...
        $("tbody").on("click", '.edit', function() {
            ...
            $("#id_panel").val($(this).attr('id_panel'));
            $("#nama_panel2").val($(this).attr('nama_panel'));
        });

        $('#edit').click(function() {
            $.ajax({
                url: '<?= base_url() ?>panel/update',
                type: "POST",
                dataType: 'json',
        ...
        $("tbody").on("click", '.hapus', function() {
            if (confirm("Apakah anda yakin ingin menghapus data tersebut?")) {
                ...
                $.ajax({
                    url: '<?= base_url() ?>panel/delete/' + id_kamar,
                    type: "POST",
                    data: {
                        id_kamar: id_kamar
                    },

        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" id="update" enctype="multipart/form-data">
                    <input type="hidden" name="id_powerbi_kategori" id="id_powerbi_kategori">
                    <div class="row">
                        <div class="form-group mb-3 col-md-12">
                            <label class="form-control-label">
                                Nama Kategori <span class="text-danger">*</span>
...
<script>
    ...
    function tampil() {
        ...
        $('#datatables').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('powerbi_kategori/get') ?>",
                "type": "POST"
            },
...
<script>
    $(document).ready(function() {
        ...
        $('#save').click(function() {
            $.ajax({
                url: '<?= base_url() ?>powerbi_kategori/insert',
                type: "POST",
                dataType: 'json',
        ...
        $("tbody").on("click", '.edit', function() {
            ...
            $("#id_powerbi_kategori").val($(this).attr('id_powerbi_kategori'));
            $("#nama_kategori2").val($(this).attr('nama_kategori'));
        });

        $('#edit').click(function() {
            $.ajax({
                url: '<?= base_url() ?>powerbi_kategori/update',
                type: "POST",
                dataType: 'json',
        ...
        $("tbody").on("click", '.hapus', function() {
            if (confirm("Apakah anda yakin ingin menghapus data tersebut?")) {
                ...
                $.ajax({
                    url: '<?= base_url() ?>powerbi_kategori/delete/' + id_kamar,
                    type: "POST",
                    data: {
                        id_kamar: id_kamar
                    },
